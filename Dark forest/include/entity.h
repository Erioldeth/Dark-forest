...
public:
	...
	void loadEntityTexture(std::string path, Uint8 r = 1, Uint8 g = 1, Uint8 b = 1) {
		...
		if(autonomous) {
			...
			entityParticle.loadParticleTexture("texture/player_particle.png", 255, 0, 0);
			entityParticle.setParticleSize(10, 10);
		}
	}
	...
	void renderEntity() {
		if(autonomous) {
			if(followPlayer) entityParticle.render(collideRegion, 24, 8, 256, 256);
			else entityParticle.render(collideRegion, 16, 4, 128, 128);
		}
		else {
			SDL_Rect region = {posX + 6,posY + ((entityTexture.textureWidth() - 20) >> 1),20,20};
			entityParticle.render(region, 25, 5, 70, 70);
		}
	...
	void free() {
		...
		for(auto& i : entitySpriteClip) i.clear();
	...
	bool collided(Entity& player) {
		...
		if(followPlayer) {
			if(rightB - (player.entityWidth() >> 2) >= leftA && leftB + (player.entityWidth() >> 2) <= rightA && bottomB > topA && topB < topA && !player.onGround) {
				Mix_HaltChannel(3);
	...
	void move(std::vector<SDL_Rect>& platform) {
		if(!motionDelay) {
			...
			for(auto& collider : platform) {
				int leftA = posX,
					rightA = posX + entityTexture.textureWidth(),
					topA = posY,
					bottomA = posY + entityTexture.textureHeight();
			...
			if(!collideBottom && posY + entityTexture.textureHeight() < SCREEN_HEIGHT) onGround = false;
			else onGround = true;
