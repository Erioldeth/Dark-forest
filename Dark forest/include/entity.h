...
public:
	...
	void setPosition(int x = 0, int y = 0) {
		if(autonomous) {
			...
			if(left) {
				collideRegion.x = posX = -entityTexture.textureWidth();
				flip = SDL_FLIP_NONE;
			}
			else {
	...
	bool collided(Entity& player) {
		...
		if(followPlayer) {
			if(leftB < rightA && rightB - (player.entityWidth() >> 2) >= leftA && leftB + (player.entityWidth() >> 2) <= rightA && bottomB > topA && topB < topA && !player.onGround) {
				...
				reaper_death.push_back(std::make_tuple(reaper_death_texture, render_pos, 199));
				setVeclocity();
				setPosition();
		...
		if(!motionDelay) {
			...
			if(autonomous) {
				...
				if(followPlayer) {
					...
					if((flip == SDL_FLIP_NONE && posX > SCREEN_WIDTH) || (flip == SDL_FLIP_HORIZONTAL && posX < -entityTexture.textureWidth())) {
						setVeclocity();
						setPosition();
