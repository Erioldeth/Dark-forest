...
const int MAX_SKULL = 4;
const int MAX_REAPER = 1;
...
Texture game_background, game_map, exploded_texture, shadow_caught_texture;
Entity player;
...
void loadMedia() {
	...
	for(int i = 0; i < MOUSE_TOTAL; i++) play.setSprite(i, 0, 0, play.buttonWidth(), play.buttonHeight());
	...
	//bot texture, animation and sound
	//skull missile
	for(auto& e : skull) {
...
void close() {
	...
	for(auto& sd : score_data) new_data << sd.first << ' ' << sd.second << "\n";
	score_data.resize(0);
