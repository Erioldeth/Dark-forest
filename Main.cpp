...
void loadMedia() {
	titleFont = TTF_OpenFont("ZombieGirlfriend.ttf", 50);
	buttonFont = TTF_OpenFont("Cabanyal-Z.ttf", 50);
	...
	for(int i = 0; i < MOUSE_TOTAL; i++) start.setSprite(i, 0, 0, start.textureWidth(), start.textureHigh());
	start.setButtonSize(BUTTON_WIDTH, BUTTON_HEIGHT);
	start.setPosition((SCREEN_WIDTH - BUTTON_WIDTH) / 2, SCREEN_HEIGHT / 4 + 50);

	instruction.textureFromText("Instruction", buttonFont, {255,255,255});
	for(int i = 0; i < MOUSE_TOTAL; i++) instruction.setSprite(i, 0, 0, instruction.textureWidth(), instruction.textureHigh());
	instruction.setButtonSize(BUTTON_WIDTH, BUTTON_HEIGHT);
	instruction.setPosition((SCREEN_WIDTH - BUTTON_WIDTH) / 2, SCREEN_HEIGHT / 4 + 120 + 50);

	high_score.textureFromText("High score", buttonFont, {255,255,255});
	for(int i = 0; i < MOUSE_TOTAL; i++) high_score.setSprite(i, 0, 0, high_score.textureWidth(), high_score.textureHigh());
	high_score.setButtonSize(BUTTON_WIDTH, BUTTON_HEIGHT);
	high_score.setPosition((SCREEN_WIDTH - BUTTON_WIDTH) / 2, SCREEN_HEIGHT / 4 + 240 + 50);
}
void close() {
	start.free();
	instruction.free();
