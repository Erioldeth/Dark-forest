...
void loadMedia() {
	...
	//start button
	start.textureFromText("Start", button_font, {255,255,255});
	...
	//high score title
	high_score_title.loadFromRenderedText("Scoreboard", deco_font, {255,0,255});
...
void refreshScore() {
	...
	score_data.resize(5);
	for(auto& i : score_data) new_data << i.first << ' ' << i.second << "\n";
}
