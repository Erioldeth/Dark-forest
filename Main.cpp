...
void loadMedia() {
	test.textureFromFile("button.png");
	for(int i = 0; i < MOUSE_TOTAL; i++) test.setSprite(i, 0, 200 * i, 300, 200);
	test.setButtonSize(BUTTON_WIDTH, BUTTON_HEIGHT);
	test.setPosition((SCREEN_WIDTH - BUTTON_WIDTH) / 2, (SCREEN_HEIGHT - BUTTON_HEIGHT) / 2);
}
void close() {
	//TTF_CloseFont();
	test.free();
