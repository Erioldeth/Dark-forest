	...
	while(true) switch(option) {
		...
		case SETTING:
			while(option == SETTING) {
				...
				for(int i = 0; i < 2; i++) switch(volume_bar[i].getCurrentState()) {
					case MOUSE_OUT:
						touch_bar[i] = false;
						volume_title[i].setTextureColor(240, 255, 255);
						volume_bar[i].setButtonColor(240, 255, 255);
						volume_block[i].setTextureColor(240, 255, 255);
						break;
					case MOUSE_IN:
						touch_bar[i] = true;
						volume_title[i].setTextureColor(147, 112, 219);
						volume_bar[i].setButtonColor(147, 112, 219);
						switch(volume_ratio[i]) {
							case 1: case 2: case 3: volume_block[i].setTextureColor(255, 69, 0); break;
							case 8: case 9: case 10: volume_block[i].setTextureColor(0, 250, 154); break;
							default: volume_block[i].setTextureColor(240, 240, 40); break;
						}
						break;
					case MOUSE_DOWN:
						int x;
				...
				if(option == SETTING) {
					...
					menu_background.renderTexture(0, 0);
					back.renderButton();
					for(int i = 0; i < 2; i++) volume_title[i].renderTexture(300, 150 + i * 60);
					for(auto& v : volume_bar) v.renderButton();
					for(int i = 0; i < 2; i++)
						for(int j = 0; j < volume_ratio[i]; j++)
							volume_block[i].renderTexture(438 + j * 22, 160 + i * 60);
					SDL_RenderPresent(renderer);
					if(touch_back || touch_bar[0] || touch_bar[1]) {
		...
		case HIGH_SCORE:
			while(option == HIGH_SCORE) {
				...
				switch(resetScore.getCurrentState()) {
					case MOUSE_OUT:
						touch_reset = false;
