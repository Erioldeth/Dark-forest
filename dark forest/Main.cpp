...
void inputScoreData() {
	...
	while(!old_data.eof()) {
		std::getline(old_data, used_name);
		old_data >> played_mode >> survived_time >> std::ws;
		if(!used_name.empty()) score_data.push_back(std::make_tuple(used_name, played_mode, survived_time));
	}
	score_data.resize(5);
}
...
void updateScoreData() {
	if(!player_name.empty()) {
		score_data.push_back(std::make_tuple(player_name, mode, survival_time));
			...
			if(option == PLAY) {
				...
				while(!confirmedName) {
					...
					if(!confirmedName) {
						...
						instrucTexture.render((SCREEN_WIDTH - instrucTexture.textureWidth()) >> 1, 210);
			...
			while(option == HIGH_SCORE) {
				...
				if(option == HIGH_SCORE) {
					...
					for(int i = 0; i < score_data.size(); i++) {
						...
						text.str("");
						text << (get<2>(score_data[i]) != 0 ? get<0>(score_data[i]) : "none") << std::string(5, ' ');
						if(get<2>(score_data[i]) == 0) text << "none";
