			...
			while(!DEAD && option == PLAY) {
				...
				if(PAUSED) {
					...
					if(PAUSED && option == PLAY) {
						...
						for(auto& rd : reaper_death) {
							int frame = get<2>(rd) / 10;
							SDL_Rect death_sprite = {192 * (frame % 5),192 * (frame / 5),192,192};
							get<0>(rd).renderTexture(get<1>(rd).x, get<1>(rd).y, &death_sprite);
						}
				...
				while(!reaper_death.empty() && get<2>(reaper_death.front()) == 200) {
					get<0>(reaper_death.front()).free();
					reaper_death.pop_front();
				}
			}
