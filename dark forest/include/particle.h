...
public:
	...
	void renderParticle(SDL_Rect& region, int totalSprite, int col, int original_width, int original_height, int range = 5) {
		for(auto& p : particle_info) {
			if(get<2>(p) == 0) {
				std::uniform_int_distribution<> horizontal_change(-range, region.w + range - particle_texture.textureWidth());
				std::uniform_int_distribution<> vertical_change(-range, region.h + range - particle_texture.textureHeight());
