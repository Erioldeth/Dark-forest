...
class Entity {
	int ID, posX, posY, velX, velY, entityVel, currentSprite, currenSubSprite, invincible_time;
...
public:
	...
	void loadEntityTexture(std::string path, Uint8 r = 1, Uint8 g = 1, Uint8 b = 1) {
		...
		if(autonomous) {
			...
			entityTexture.setTextureColor(color_code[0][0], color_code[0][1], color_code[0][2]);
			entityParticle.loadParticleTexture("texture/player_particle.png");
			entityParticle.setParticleColor(color_code[1][0], color_code[1][1], color_code[1][2]);
	...
	void renderEntity() {
		...
		entityTexture.renderTexture(posX, posY, &entitySpriteClip[currentSprite][currenSubSprite], 0.0, nullptr, flip);
	...
	void move(std::vector<SDL_Rect>& platform) {
		...
		currenSubSprite = (currenSubSprite + 1) % (int)entitySpriteClip[currentSprite].size();
	...
	void move(Entity& player) {
		if(!motionDelay) {
			if(play_sound(mt) == 128 && ((followPlayer && !Mix_Playing(3)) || (!followPlayer && !Mix_Playing(4)))) {
				...
				loadMoveSound(path, 3 * volume_ratio[1]);
			...
			if(followPlayer) {
				...
				std::array<double, 2> vec{double(player_center.x - center.x),double(player_center.y - center.y)};
