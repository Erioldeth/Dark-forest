...
class Entity {
	...
	bool autonomous, followPlayer, onGround, jump, invincible, sit, motionDelay;
...
public:
	...
	bool collided(Entity& player) {
		...
		if(followPlayer) {
			...
			if(rightB > leftA && leftB < rightA && bottomB > topA && topB < topA && player.velY > 0) {
				Mix_HaltChannel(3);
		...
		if(!motionDelay) {
			...
			if(followPlayer) {
				if(create_sound == 128 && !Mix_Playing(3)) {
					...
					loadMoveSound(path, reaper_move_volume);
				...
				if(create_sound == 128 && !Mix_Playing(4)) {
					...
					loadMoveSound(path, skuLL_move_volume);
